...
class CanvasRenderer {
    ...
    async preloadSocialIcons(iconNames) {
        const loadPromises = iconNames.map(async (iconName) => {
            if (this.socialIconImageCache.has(iconName))
                return;
    ...
    drawMathBlock(ctx, layout, x, y, maxWidth) {
        if (!layout.image || typeof layout.image === 'string')
            return;
    ...
    drawTableGrid(ctx, layout, x, y, config, templateId) {
        ...
        for (let r = 0; r < rows.length; r++) {
            ...
            for (let c = 0; c < row.cells.length; c++) {
                ...
                this.drawStyledLines(ctx, cell.lines, currentX + cellPaddingX, textY, config, templateId, cellWidth - cellPaddingX * 2, cell.align);
                currentX += cellWidth;
            }
            ctx.beginPath();
    ...
    drawCodeBlock(ctx, layout, x, y, config, templateId, maxWidth) {
        ...
        if (template && template.getTextStyles) {
            ...
            if (styles && styles.codeBgColor)
                bgColor = styles.codeBgColor;
        }
        ctx.save();
        CanvasUtils.drawRoundedRect(ctx, x, y, maxWidth, contentHeight, 8, bgColor, true, borderColor);
        this.drawStyledLines(ctx, layout.lines, x + paddingX, y + paddingY, config, templateId, maxWidth - (paddingX * 2), layout.align);
        ctx.restore();
    }
    /**
     * 绘制正文中的图片
    ...
    drawSegment(ctx, segment, x, y, config, templateId) {
        ...
        if (template && template.getTextStyles) {
            ...
            if (styles) {
                if (styles.textColor && !segment.color)
                    textColor = styles.textColor;
                if (styles.highlightColor)
                    highlightColor = styles.highlightColor;
                if (styles.codeBgColor)
                    codeBgColor = styles.codeBgColor;
            }
        }
        const letterSpacing = Number(config.letterSpacing) || 0;
    ...
    drawSignature(ctx, config, width, height, templateId) {
        ...
        if (sigStyle === 'terminal') {
            ...
            ctx.moveTo(width - startX - lineWidth, y);
