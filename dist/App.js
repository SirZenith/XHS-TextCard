...
class App {
    ...
    bindEvents() {
        ...
        this.elements.previewList.addEventListener('scroll', () => requestAnimationFrame(() => this.updateActiveIndicator()));
    ...
    updateActiveIndicator() {
        if (!this.elements.previewIndicators)
            return;
    ...
    async generatePreview() {
        ...
        try {
            ...
            if (generationId !== this.previewGenerationId)
                return;
            ...
            const renderPromises = this.splitPages.map(async (pageLayouts, index) => {
                const previewItem = await this.previewGenerator.createPreviewItem(pageLayouts, index, this.splitPages.length, this.currentTemplate, templateConfig, (idx) => this.downloadSingleImage(idx));
                return previewItem;
            });
    ...
    resetTemplate() {
        ...
        if (template) {
            ...
            // 使用深度克隆恢复初始配置
            const resetConfig = JSON.parse(JSON.stringify(template.config));
