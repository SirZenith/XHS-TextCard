...
class EditorController {
    constructor() {
        ...
        this.onExportFormatChange = null;
        this.pickrs = {};
    ...
    initGradientEditor() {
        ...
        document.getElementById('gradient-angle')?.addEventListener('input', (e) => {
            ...
            if (label)
                label.textContent = deg + 'deg';
            this.updateGradientFromPickrs();
        });
        document.addEventListener('click', (e) => {
            if (!popup.contains(e.target) && !plusParent.contains(e.target) && !e.target.closest('.pcr-app')) {
    ...
    parseCurrentGradient() {
        ...
        if (angleMatch) {
            ...
            if (angleInput)
                angleInput.value = angleMatch[1];
            const label = document.getElementById('gradient-angle-value');
            if (label)
                label.textContent = angleMatch[0];
        }
    }
    ...
    setBgMode(mode) {
        ...
        if (solidPresets)
            solidPresets.style.display = mode === 'solid' ? 'flex' : 'none';
        if (gradientPresets)
            gradientPresets.style.display = mode === 'gradient' ? 'flex' : 'none';
        if (bgPickrRoot)
            bgPickrRoot.style.display = mode === 'solid' ? 'block' : 'none';
    ...
    bindEvents() {
        ...
        document.querySelectorAll('.format-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                // 通知App更新导出格式
                if (this.onExportFormatChange)
                    this.onExportFormatChange(format || 'png');
            });
        });
        const presetGroups = [
    ...
    getControlElement(cfg) {
        if (this.elements[cfg.key + 'Input'])
            return this.elements[cfg.key + 'Input'];
        if (this.elements[cfg.key + 'Check'])
            return this.elements[cfg.key + 'Check'];
        if (this.elements[cfg.key + 'Select'])
            return this.elements[cfg.key + 'Select'];
    ...
    updateUIControl(cfg, val) {
        ...
        let displayVal = val;
        if (['fontSize', 'textPadding', 'letterSpacing', 'coverFontSize'].includes(cfg.key))
            displayVal = val + 'px';
        else if (cfg.key.endsWith('Scale'))
            displayVal = val + 'x';
        if (label)
            label.textContent = String(displayVal);
    ...
    setConfig(config) {
        ...
        if (config.bgColor) {
            ...
            if (!config.bgColor.startsWith('linear-gradient'))
                this.pickrs.bgColor?.setColor(config.bgColor, true);
        }
        if (config.textColor) {
        ...
        if (config.watermarkColor)
            this.pickrs.watermarkColor?.setColor(config.watermarkColor, true);
        if (config.signatureColor)
            this.pickrs.signatureColor?.setColor(config.signatureColor, true);
        // 更新封面图片提示
        const fileNameHint = document.getElementById('cover-file-name');
        if (fileNameHint) {
    ...
    updateActivePreset(type, color) {
        ...
        let containerId;
        if (type === 'bg-color')
            containerId = 'bg-color-presets';
        else if (type === 'text-color')
            containerId = 'text-color-presets';
        else if (type === 'accent-color')
            containerId = 'accent-color-presets';
        if (!containerId)
            return;
    ...
    notifyConfigChange() { if (this.onConfigChange && this.currentConfig)
