...
class CanvasTextEngine {
    ...
    tokenizePythonCode(code) {
        ...
        const tokenRule = new RegExp(`(#.*)|(\"\"\"[\\\\s\\\\S]*?\"\"\"|'''[\\\\s\\\\S]*?'''|\"(?:\\\\\\\\.|[^\"\\\\\\\\])*\"|'(?:\\\\\\\\.|[^'\\\\\\\\])*')|\\b(\\d+(?:\\.\\d+)?)\\b|\\b(${keywordPattern})\\b|\\b(${builtInPattern})\\b|\\b([A-Za-z_]\\w*)(?=\\s*\\()`, 'g');
        const segments = [];
        let lastIndex = 0;
        let match;
    ...
    highlightCode(code, lang) {
        ...
        const walk = (node, scope = '') => {
            if (node.nodeType === Node.TEXT_NODE) {
                if (node.nodeValue)
                    segments.push({ text: node.nodeValue, color: this.getCodeColor(scope) });
                return;
            }
    ...
    async renderMermaid(text, maxHeight = this.config.maxBlockHeight || Infinity) {
        if (!text)
            return this.createRenderErrorLayout('Mermaid 图表内容为空');
        ...
        try {
            ...
            if (!image)
                return this.createRenderErrorLayout('Mermaid 图表载入失败');
    ...
    async layoutTable(token) {
        ...
        for (let c = 0; c < C; c++) {
            ...
            const lines = await this.layoutInlineText(cell.tokens || [{ type: 'text', text: cell.text }], cellWidth - (cellPaddingX * 2), { fontWeight: '700' });
    ...
    splitTableLayout(layout, availableHeight) {
        if (!layout || layout.type !== 'table-grid' || !layout.rows || layout.rows.length < 3)
            return null;
        ...
        if (splitIndex >= layout.rows.length)
            return null;
        return {
        ...
        const processTokens = async (tokens, currentStyle) => {
            for (const token of tokens) {
                ...
                if (token.type === 'codespan' || token.type === 'code')
                    style.isCode = true;
                if (token.type === 'del' || token.type === 'strikethrough')
    ...
    splitLayout(layout, availableHeight) {
        if (!layout.lines || !Array.isArray(layout.lines) || layout.lines.length === 0)
            return null;
        ...
        const part2 = { ...layout, lines: part2Lines, marginTop: 0, height: part2ContentHeight + (layout.marginBottom || 0) };
        if (layout.type === 'list-item') {
            part2.type = 'paragraph';
            part2.prefix = '';
        }
        return { part1, part2 };
    }
