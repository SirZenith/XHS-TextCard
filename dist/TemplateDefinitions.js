...
const TemplateDefinitions = {
    ...
    'polaroid': {
        ...
        getTextStyles: (segment, config) => {
            ...
            if (segment.isHighlight)
                return { textColor: textColor, highlightColor: CanvasUtils.hexToRgba(accentColor, 0.2) };
            return { textColor };
        }
    },
    /**
    ...
    'elegant-book': {
        ...
        getTextStyles: (segment, config) => {
            ...
            if (segment.fontWeight === '700' || segment.fontWeight === '800' || segment.headingLevel)
                return { textColor: accentColor };
            if (segment.isHighlight)
                return { textColor: accentColor, highlightColor: CanvasUtils.hexToRgba(accentColor, 0.1) };
            return { textColor };
        }
    },
    'ios-memo': {
    ...
    'minimalist-magazine': {
        ...
        drawForeground: (ctx, width, height, index, totalCount, config) => {
            ...
            ctx.textAlign = 'right';
            ctx.fillText('COLLECTION // VOL. 2026', width - 45, 75);
            ctx.restore();
    ...
    'deep-night': {
        ...
        drawForeground: (ctx, width, height, index, totalCount, config) => {
            ...
            ctx.textAlign = 'right';
            ctx.fillText('// THOUGHT MODE ON', width - 25, 25);
    ...
    'pro-doc': {
        ...
        drawTextAreaBackground: (ctx, rect, config) => {
            ...
            ctx.beginPath();
            ctx.arc(winX + 20, btnY, 5, 0, Math.PI * 2);
            ctx.fill();
            ctx.fillStyle = '#FFBD2E';
            ctx.beginPath();
            ctx.arc(winX + 38, btnY, 5, 0, Math.PI * 2);
            ctx.fill();
            ctx.fillStyle = '#27C93F';
            ctx.beginPath();
            ctx.arc(winX + 56, btnY, 5, 0, Math.PI * 2);
            ctx.fill();
    ...
    'cinematic-film': {
        ...
        _getFilmGrain: function (width, height) {
            if (this._filmGrainCache)
                return this._filmGrainCache;
            const canvas = document.createElement('canvas');
            canvas.width = width;
            canvas.height = height;
            const tCtx = canvas.getContext('2d');
