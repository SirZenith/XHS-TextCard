 ...
 */
const TemplateDefinitions: TemplateDefinitionsMap = {
    /**
     * 内部页码绘制辅助工具
     */
    _drawPageNumber: (ctx: CanvasRenderingContext2D, width: number, height: number, index: number, totalCount: number, config: TemplateConfig, options: PageNumberOptions = {}) => {
     ...
     * 噪点纹理缓存
     */
    _noiseTextureCache: new Map<string, HTMLCanvasElement>(),

    /**
     * 生成噪点纹理
     */
    _getNoiseTexture: (width: number, height: number) => {
        const key = `${width}x${height}`;
        const cachedNoise = TemplateDefinitions._noiseTextureCache.get(key);
        if (cachedNoise) {
            return cachedNoise;
        }
     ...
     * 纸张纹理缓存
     */
    _paperTextureCache: new Map<string, HTMLCanvasElement>(),

    /**
     * 生成纸张纹理 (深度优化版：包含噪点、长纤维、以及随机纸浆感)
     */
    _getPaperTexture: (width: number, height: number) => {
        const key = `${width}x${height}`;
        const cachedPaper = TemplateDefinitions._paperTextureCache.get(key);
        if (cachedPaper) {
            return cachedPaper;
        }
