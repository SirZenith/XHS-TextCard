...
export class CanvasRenderer {
     ...
     * 预加载社交图标
     */
    async preloadSocialIcons(iconNames: string[]): Promise<void> {
        const loadPromises = iconNames.map(async (iconName) => {
            ...
            try {
                ...
                if (img) this.socialIconImageCache.set(iconName, img);
            } catch (e) {
    ...
    async render(options: RenderOptions): Promise<HTMLCanvasElement> {
        const {
        ...
        // 1. 预加载必要的图片 (如封面、正文插图)
        const imageLoadPromises: Promise<HTMLImageElement | null>[] = [];
        layouts.forEach(layout => {
            if (layout.type === 'cover' && layout.image) {
                imageLoadPromises.push(this.loadImage(layout.image as string));
     ...
     * 下 40%: 标题
     */
    drawCoverContent(ctx: CanvasRenderingContext2D, layout: LayoutBlock, config: TemplateConfig, width: number, height: number, templateId: string) {
        const img = this.imageCache.get(layout.image as string);
        ...
        const fontSize = Number(config.coverFontSize) || 48;
        ...
        const lineHeight = fontSize * (Number(config.coverLineHeight) || 1.4);
        const totalTextHeight = lines.length * lineHeight;
        // 垂直居中于下半部分
        const startY = textY + (textH / 2) - (totalTextHeight / 2) + (fontSize * 0.4);
     ...
     * 绘制辅助网格，用于排版对齐查看
     */
    drawGridLayout(ctx: CanvasRenderingContext2D, textAreaRect: ContentBox, layouts: LayoutBlock[]) {
        ctx.save();
    ...
    drawWatermark(ctx: CanvasRenderingContext2D, config: TemplateConfig, width: number, height: number) {
        ctx.save();
    ...
    drawTextContent(ctx: CanvasRenderingContext2D, layouts: LayoutBlock[], config: TemplateConfig, textAreaRect: ContentBox, templateId: string) {
        let currentY = textAreaRect.y;
        ctx.save();
    ...
    drawMathBlock(ctx: CanvasRenderingContext2D, layout: LayoutBlock, x: number, y: number, maxWidth: number) {
        if (!layout.image || typeof layout.image === 'string') return;
    ...
    drawMermaidBlock(ctx: CanvasRenderingContext2D, layout: LayoutBlock, x: number, y: number, maxWidth: number) {
        if (!layout.image || typeof layout.image === 'string') return;
    ...
    drawRenderError(ctx: CanvasRenderingContext2D, layout: LayoutBlock, x: number, y: number, maxWidth: number, config: TemplateConfig) {
    ...
    drawTableGrid(ctx: CanvasRenderingContext2D, layout: LayoutBlock, x: number, y: number, config: TemplateConfig, templateId: string) {
        ...
        const baseLineHeight = (Number(config.fontSize) || 16) * (Number(config.lineHeight) || 1.6);
    ...
    drawCodeBlock(ctx: CanvasRenderingContext2D, layout: LayoutBlock, x: number, y: number, config: TemplateConfig, templateId: string, maxWidth: number) {
     ...
     * 绘制正文中的图片
     */
    drawInlineImage(ctx: CanvasRenderingContext2D, layout: LayoutBlock, x: number, y: number) {
        const img = this.imageCache.get(layout.src as string);
        if (img) {
    ...
    drawStyledLines(ctx: CanvasRenderingContext2D, lines: Array<TextSegment[] | TextSegment>, startX: number, startY: number, config: TemplateConfig, templateId: string, maxWidth: number | null = null, align: string = 'left') {
        ...
        for (const lineSegments of lines) {
            ...
            if (Array.isArray(lineSegments)) {
                if (lineSegments.length > 0) {
                    maxFontSize = Math.max(...lineSegments.map(s => Number(s.fontSize) || configFontSize));
                }
            } else if (lineSegments && lineSegments.fontSize) {
                maxFontSize = Number(lineSegments.fontSize);
            }

            const lineHeight = maxFontSize * (Number(config.lineHeight) || 1.6);
    ...
    drawSignature(ctx: CanvasRenderingContext2D, config: TemplateConfig, width: number, height: number, templateId: string) {
     ...
     */
    getSocialIconCanvas(iconName: string, size: number): HTMLCanvasElement | null {
        const cacheKey = iconName + '_' + size;
        const cachedCanvas = this.socialIconCanvasCache.get(cacheKey);
        if (cachedCanvas) {
            return cachedCanvas;
        }
     ...
     */
    drawSocialIcons(ctx: CanvasRenderingContext2D, config: TemplateConfig, width: number, height: number, templateId: string) {
