...
import type { ContentBox, PageNumberOptions, TemplateConfig, TextSegment } from '../types';
import { Template, TextStyle } from '../templates/type';

export namespace TemplateDefinitions {
    /**
     * 内部页码绘制辅助工具
     */
    const drawPageNumber = (ctx: CanvasRenderingContext2D, width: number, height: number, index: number, totalCount: number, config: TemplateConfig, options: PageNumberOptions = {}) => {
     ...
     * 噪点纹理缓存
     */
    const noiseTextureCache = new Map<string, HTMLCanvasElement>()

    /**
     * 生成噪点纹理
     */
    const getNoiseTexture = (width: number, height: number) => {
     ...
     * 纸张纹理缓存
     */
    const paperTextureCache = new Map<string, HTMLCanvasElement>();

    /**
     * 生成纸张纹理 (深度优化版：包含噪点、长纤维、以及随机纸浆感)
     */
    const getPaperTexture = (width: number, height: number) => {
        const key = `${width}x${height}`;
    ...
    const styleMap: Record<string, Template> = {
        /**
         * 书籍内页 - 模拟真实的单页书籍装帧感
        ...
        'elegant-book': {
            ...
            drawBackground: (ctx: CanvasRenderingContext2D, width: number, height: number, config: TemplateConfig) => {
                ctx.save();
                ...
                // 2. 绘制离屏缓存的高级纸张纹理
                const texture = getPaperTexture(width, height);
            ...
            drawForeground: (ctx: CanvasRenderingContext2D, width: number, height: number, index: number, totalCount: number, config: TemplateConfig) => {
                const padding = Number(config.textPadding) || 55;
                ctx.save();
                ...
                // 页码：使用统一的物理边角定位
                drawPageNumber(ctx, width, height, index, totalCount, config, {
                    color: fixedMutedInk,
        ...
        'ios-memo': {
            ...
            drawTextAreaBackground: (ctx: CanvasRenderingContext2D, rect: ContentBox, config: TemplateConfig) => {
        ...
        'minimalist-magazine': {
            ...
            drawForeground: (ctx: CanvasRenderingContext2D, width: number, height: number, index: number, totalCount: number, config: TemplateConfig) => {
                ...
                ctx.restore();
                drawPageNumber(ctx, width, height, index, totalCount, config, { x: width - 45, y: height - 35 });
            },
            getTextStyles: (segment: TextSegment, config: TemplateConfig): TextStyles => {
        ...
        'aura-gradient': {
            ...
            drawBackground: (ctx: CanvasRenderingContext2D, width: number, height: number) => {
                ...
                // 绘制缓存的噪点纹理
                const noise = getNoiseTexture(width, height);
        ...
        'deep-night': {
            ...
            drawForeground: (ctx: CanvasRenderingContext2D, width: number, height: number, index: number, totalCount: number, config: TemplateConfig) => {
                ...
                ctx.restore();
                drawPageNumber(ctx, width, height, index, totalCount, config, { color: 'rgba(255, 255, 255, 0.3)', textAlign: 'left', x: 25, y: 25 });
            },
            getTextStyles: (segment: TextSegment, config: TemplateConfig): TextStyles => {
         ...
         */
        'cinematic-film': {
            _filmGrainCache: null as HTMLCanvasElement | null,
            _getFilmGrain: function (width: number, height: number) {
            ...
            drawBackground: (ctx: CanvasRenderingContext2D, width: number, height: number, config: TemplateConfig) => {
                ...
                // 4. 绘制 35mm 胶片颗粒纹理
                const filmTemplate = styleMap['cinematic-film'];
         ...
         */
        'starry-night': {
            _starCache: null as HTMLCanvasElement | null,
            getStars: function (width: number, height: number) {
            ...
            drawBackground: (ctx: CanvasRenderingContext2D, width: number, height: number, config: TemplateConfig) => {
                ...
                // 绘制星星 - 使用模板内部方法
                const starryTemplate = styleMap['starry-night'];
